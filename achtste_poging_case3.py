...
for _, row in valid_metro_data.iterrows():
    # **Verwijder puntjes uit getallen om numerieke waarden correct te krijgen**
    entries = pd.to_numeric(row[entries_col].replace('.', '', regex=True), errors="coerce")
    exits = pd.to_numeric(row[exits_col].replace('.', '', regex=True), errors="coerce")
...
for _, row in valid_metro_data.iterrows():
    ...
    # **Verwijder puntjes en zet om naar numeriek**
    entries = pd.to_numeric(row[entries_col].replace('.', '', regex=True), errors="coerce")
    exits = pd.to_numeric(row[exits_col].replace('.', '', regex=True), errors="coerce")
    ...
    if pd.notnull(busy_value):
        ...
        # **Debugging print**
        st.write(f"{station_name}: {entries} entries, {exits} exits → {busy_value} passagiers")
