...
# Coördinaten dictionary voor metro
stations_dict = {
    row["Station"]: (row["Latitude"], row["Longitude"]) 
    for _, row in metro_stations_data.iterrows()
}

# Niet-numerieke tekens verwijderen bij 'AnnualisedEnEx' en omzetten naar float
...
# Vermenigvuldig de Entries en Exits met 1000
entry_exit_cols = [
...
# Maak een nieuwe kolom 'TotalEnEx' (optelsom over alle entries/exits)
metro_data["TotalEnEx"] = metro_data[entry_exit_cols].sum(axis=1)

# Kwantiel-berekening (low, mid)
...
with tab3:
    ...
    # Mapping voor netjes tonen in tabel
    column_mapping = {
    ...
    # Datumselectie
    datum = st.date_input("Selecteer een datum in 2021",
    ...
    # Weeknummer toevoegen aan dataset
    ...
    if not filtered_data_week.empty:
        ...
        # Datum formatteren
        filtered_data_week_reset['Date'] = pd.to_datetime(filtered_data_week_reset['Date']).dt.strftime('%d %B %Y')

        # Kolommenvolgorde
        kolommen = [
            'Date', 'Aantal Verhuurde Fietsen',
            'Gemiddelde Temperatuur (°C)', 'Minimale Temperatuur (°C)', 'Maximale Temperatuur (°C)',
            'Neerslag (mm)', 'Sneeuwval (cm)', 'Windrichting (°)', 'Windsnelheid (m/s)',
            'Windstoten (m/s)', 'Luchtdruk (hPa)', 'Zonduur (uren)'
        ]
...
# ==========  REGRESSIE-DEEL  =========
# =====================================
# (Hier vervangen we het oude regressiedeel door de versie met Nederlandse weerfactoren.)

# Data opnieuw inladen voor regressie (of gebruik dezelfde boven; hier is het expliciet)
fiets_rentals = pd.read_csv('fietsdata2021_rentals_by_day.csv')
weer_data = pd.read_csv('weather_london.csv')
...
# Titel voor regressie
st.title("Regressieanalyse: Fietsverhuur en Weer")

# Mapping van weerfactoren (NL -> Engelse kolomnamen), wspd is verwijderd
...
# Dropdown met Nederlandse namen
weerfactor_nl = st.selectbox("Kies een weerfactor:", list(weerfactoren_mapping.keys()))

# Pak de echte kolomnaam
originele_kolomnaam = weerfactoren_mapping[weerfactor_nl]
...
if originele_kolomnaam not in combined_df.columns:
    ...
    # X (weerfactor) en Y (fietsverhuur)
    x = combined_df[originele_kolomnaam]
    ...
    ax.set_xlabel(weerfactor_nl)  # NL label
