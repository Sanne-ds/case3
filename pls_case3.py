...
with tab3:
    ...
    # Voeg de regressielijn visualisatie toe aan het weer-tabblad
    ...
    # Nederlandse labels en mapping naar Engelse kolomnamen
    weerfactor_opties = {
        "Gemiddelde Temperatuur (°C)": "tavg",
        "Minimale Temperatuur (°C)": "tmin",
        "Maximale Temperatuur (°C)": "tmax",
        "Neerslag (mm)": "prcp",
        "Windsnelheid (m/s)": "wspd"
    }

    # Dropdown met Nederlandse labels
    gekozen_label = st.selectbox("Kies een weerfactor:", list(weerfactor_opties.keys()))
    weerfactor = weerfactor_opties[gekozen_label]  # Haal de juiste Engelse kolomnaam op
    ...
    ax.set_xlabel(gekozen_label)  # Gebruik de Nederlandse label
    ax.set_ylabel("Aantal Fietsverhuringen")
    ax.set_title(f"Regressie: {gekozen_label} vs. Fietsverhuur\nR² = {r_squared:.2f}")
